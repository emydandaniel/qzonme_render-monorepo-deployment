...
const QuizCreation: React.FC = () => {
  ...
  React.useEffect(() => {
    console.log('🚀 QUIZ EDITOR: Component mounted - starting sessionStorage debug...');
    ...
    console.log('🔍 QUIZ EDITOR: === FULL SESSIONSTORAGE DEBUG ===');
    console.log('🔍 QUIZ EDITOR: autoCreateMode =', autoCreateMode);
    console.log('🔍 QUIZ EDITOR: requiresReview =', requiresReviewFlag);
    console.log('🔍 QUIZ EDITOR: hasGeneratedQuestions =', hasGeneratedQuestions);
    console.log('🔍 QUIZ EDITOR: generatedQuestions length =', generatedQuestionsRaw?.length || 0);
    console.log('🔍 QUIZ EDITOR: generationMetadata length =', generationMetadataRaw?.length || 0);
    console.log('🔍 QUIZ EDITOR: All sessionStorage keys:', Object.keys(sessionStorage));
    
    if (generatedQuestionsRaw) {
      try {
        const parsedQuestions = JSON.parse(generatedQuestionsRaw);
        console.log('🔍 QUIZ EDITOR: Successfully parsed questions count:', parsedQuestions?.length || 0);
        console.log('🔍 QUIZ EDITOR: First question sample:', parsedQuestions?.[0]);
      } catch (e) {
        console.error('❌ QUIZ EDITOR: Failed to parse generatedQuestions:', e);
      }
    }
    ...
    console.log('🎯 QUIZ EDITOR: Activation decision - shouldActivateAutoCreate =', shouldActivateAutoCreate);
    console.log('🎯 QUIZ EDITOR: Setting isAutoCreateMode =', shouldActivateAutoCreate);
    console.log('🎯 QUIZ EDITOR: Setting requiresReview =', shouldActivateAutoCreate);
    ...
    if (!shouldActivateAutoCreate && (autoCreateMode || requiresReviewFlag)) {
      ...
      console.log('🧹 QUIZ EDITOR: Clearing stale auto-create flags - conditions not met');
      ...
      if (questions.length > 0 && isAutoGenerated) {
        console.log('🧹 QUIZ EDITOR: Clearing auto-generated questions from state');
        setQuestions([]);
  ...
  React.useEffect(() => {
    console.log('� QUIZ EDITOR: Second useEffect triggered - checking for auto-generated questions...');
    console.log('🔍 QUIZ EDITOR: isAutoCreateMode =', isAutoCreateMode);
    console.log('🔍 QUIZ EDITOR: requiresReview =', requiresReview);
    ...
    console.log('🔍 QUIZ EDITOR: generatedQuestions exists =', !!generatedQuestions);
    console.log('🔍 QUIZ EDITOR: generationMetadata exists =', !!generationMetadata);
    console.log('🔍 QUIZ EDITOR: generatedQuestions raw data length =', generatedQuestions?.length || 0);
    ...
    console.log('🎯 QUIZ EDITOR: Should load auto questions =', shouldLoadAutoQuestions);
    
    if (shouldLoadAutoQuestions) {
      try {
        console.log('🔍 QuizEditor: Loading auto-generated questions...');
        const parsedQuestions = JSON.parse(generatedQuestions);
        
        if (Array.isArray(parsedQuestions) && parsedQuestions.length > 0) {
          console.log('✅ QuizEditor: Loading', parsedQuestions.length, 'auto-generated questions');
          ...
          console.log('🔍 QuizEditor: Loading first AI question into form for review:', firstQuestion);
          console.log('🔍 QuizEditor: Question text:', firstQuestion.text);
          console.log('🔍 QuizEditor: Question options:', firstQuestion.options);
          console.log('🔍 QuizEditor: Question correctAnswers:', firstQuestion.correctAnswers);
          ...
          console.log('🔍 QuizEditor: Set form data - Text:', firstQuestion.text);
          ...
          console.log('✅ QuizEditor: Auto-review mode activated');
        }
      } catch (e) {
        console.error('❌ QuizEditor: Error loading auto-generated questions:', e);
        toast({
        ...
        if (savedQuestions) {
          ...
          if (savedMetadata) {
            try {
              ...
              console.log('❌ QuizEditor: Invalid saved metadata, treating as suspicious');
              shouldLoadSavedQuestions = false;
            }
          } else {
            // No metadata means we can't trust the source - could be old auto-generated questions
            console.log('❌ QuizEditor: No metadata for saved questions, skipping load to prevent auto-generated contamination');
            shouldLoadSavedQuestions = false;
          }
          
          if (shouldLoadSavedQuestions) {
            console.log('✅ QuizEditor: Loading', parsedQuestions.length, 'verified manual draft questions');
            setQuestions(parsedQuestions);
          } else {
            // Clear potentially contaminated localStorage data
            console.log('🧹 QuizEditor: Clearing potentially contaminated draft questions');
            localStorage.removeItem('qzonme_draft_questions');
            localStorage.removeItem('qzonme_draft_metadata');
          }
        }
      } catch (e) {
        console.error('❌ QuizEditor: Error loading saved questions:', e);
  ...
  React.useEffect(() => {
    console.log('🔍 QuizEditor: Current questions state:', questions);
    console.log('🔍 QuizEditor: Questions length:', questions.length);
    console.log('🔍 QuizEditor: Is auto-generated:', isAutoGenerated);
    console.log('🔍 QuizEditor: Is in auto-review mode:', isInAutoReviewMode);
    console.log('🔍 QuizEditor: Current auto-review index:', currentAutoReviewIndex);
  ...
  const handleStartFresh = () => {
    console.log('🔄 Starting fresh - clearing all questions and data');
  ...
  const handleEditQuestion = (index: number) => {
    ...
    if (isInAutoReviewMode) {
      ...
      console.log("🔄 Exited auto-review mode to edit question", index + 1);
  ...
  const handleMarkAsReviewed = (index: number) => {
    ...
    if (requiresReview) {
      setReviewedQuestions(prev => new Set([...Array.from(prev), question.id]));
      console.log(`✅ Question ${question.id} marked as reviewed (viewed)`);
      ...
      {(isInAutoReviewMode || (isAutoGenerated && questions.length > 0) || requiresReview) && (
        <Alert className="mb-6 border-blue-200 bg-blue-50">
          ...
          <AlertTitle className="text-blue-800">
            {isInAutoReviewMode ? `📝 Auto-Review Mode: Question ${currentAutoReviewIndex + 1} of ${questions.length}` : "Questions Auto-Generated!"}
          </AlertTitle>
          <AlertDescription className="text-blue-700 space-y-2">
            {isInAutoReviewMode ? (
              ...
              <div>
                ✅ {questions.length} questions have been generated below. You can edit them, add more questions using the form, or remove any you don't want.
              </div>
            )}
            {requiresReview && (

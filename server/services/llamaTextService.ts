...
function ensureAnswerVariety(questions: GeneratedQuestion[]): GeneratedQuestion[] {
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    do {
      ...
      // Only avoid immediate consecutive duplicates and obvious patterns
      (i > 0 && randomAnswer === shuffledAnswers[i - 1]) && // No consecutive duplicates
      attempts < 5 // Give up after 5 attempts to avoid infinite loops
    );
